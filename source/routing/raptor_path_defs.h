...
void handle_connection(const size_t countb, const navitia::type::idx_t current_stop_point_idx, Visitor& v,
                       bool clockwise, const RAPTOR &raptor_) {
    auto departure = raptor_.data.pt_data->stop_points[current_stop_point_idx];
    auto destination_stop_point_idx = raptor_.labels[countb][current_stop_point_idx].boarding_stop_point_transfer;
    auto destination = raptor_.data.pt_data->stop_points[destination_stop_point_idx];
    auto connections = departure->stop_point_connection_list;
    auto l = raptor_.labels[countb][current_stop_point_idx].dt_transfer;
    ...
    if(it == connections.end()) {
        auto r2 = raptor_.labels[countb][destination_stop_point_idx];
        if(clockwise) {
...
void handle_vj(const size_t countb, navitia::type::idx_t current_stop_point_idx, Visitor& v,
    ...
    DateTimeUtils::update(workingDate, current_st->section_end_time(!clockwise, workingDate), clockwise);

    while(boarding_jpp_idx != current_journey_pattern_point_idx) {
        ...
        current_journey_pattern_point_idx = current_st->journey_pattern_point->idx;
    }
    // There is a side effect on workingDate caused by workingDate
    auto departure_arrival = handle_st(current_st, workingDate, clockwise, raptor_.data);
    v.loop_vj(current_st, departure_arrival.first, departure_arrival.second);
...
void read_path(Visitor& v, type::idx_t destination_idx, size_t countb, bool clockwise, bool disruption_active,
    ...
    while (countb>0) {
        ...
        handle_connection(countb, current_stop_point_idx, v, clockwise, raptor_);
        current_stop_point_idx = raptor_.labels[countb][current_stop_point_idx].boarding_stop_point_transfer;
        v.final_step(current_stop_point_idx, countb, raptor_.labels);
    }
}
